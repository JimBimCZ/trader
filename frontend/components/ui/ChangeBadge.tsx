 ...
 * for the places where the number is the row's headline rather than one
 * column among many.
 */
export function ChangeBadge({
  ...
  const skin = pill
    ? `rounded-full px-2 py-0.5 text-xs font-semibold ${tonePillClass(value)}`
    : `text-xs font-semibold ${toneClass(value)}`;
