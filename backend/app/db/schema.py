...
#: The same schema in Postgres, for the serverless deployment.
#:
#: Three things differ, and only three. REAL is a 4-byte float in Postgres and
#: would round a cash balance visibly, so money and quantities are DOUBLE
#: PRECISION. Every table gains an explicit `seq`, because Postgres has no
#: implicit rowid and three queries order by a timestamp that ties. And the
#: whole script runs as one statement rather than through executescript.
POSTGRES_SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS users_profile (
    id           TEXT PRIMARY KEY,
